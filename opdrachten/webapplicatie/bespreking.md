...
Eerst is een database schema (bijlage 1) gemaakt waardoor een overzicht wordt verkregen van de data en relaties in de applicatie. Voor connectie mogelijk is moet de configuratie file aangepast worden in de config folder van het laravel project: 
...
Momenteel ondersteunt Laravel 4 database systemen: MySQL, Postgres, SQLite en SQL server. In dit project is gebruik gemaakt van een MySQL database. 

##### migrations

Migrations laten toe om gemakkelijk in een team met een databank te werken. Telkens een nieuwe table moet aangemaakt worden moet er een migration file worden toegevoegd. Als nu een andere developer wilt werken aan het project zal hij zijn database kunnen bijwerken of aanmaken door het `php artisan migrate` command uit te voeren. Ook een handig voordeel met het werken van migrations is dat als er iets misloopt met de databank dan kan deze helemaal opnieuw aangemaakt worden met Artisan. Migrations zijn een soort van version control van de databank , het houd een geschiedenis bij in de databank van wat er allemaal al gemigrate is. Als er nu nieuwe migrations bestaan die nog niet in deze tabel staan dan zullen deze migrations uitgevoerd worden bij het migraten met Artisan.
...
Om de applicatie te testen is het handig dat er wat test data gebruikt kan worden. Dit kan verwezenlijkt worden door het gebruik van seeders. Een seeder klasse kan gegenereerd worden met Artisan, deze klasse bevat maar één functie genaamd run en zal aangesproken worden als het artisan seed command word uitgevoerd. In deze functie word er data in de database opgeslagen door gebruik van de Laravel query builder, men kan ook grote hoeveelheden van data random genereren door gebruik van de model factories. In het begin van het ontwikkelen is gebruik gemaakt van seeders omdat er dan al data uit de database gehaald kon worden.

Hierna zijn de models aangemaakt zodat de ORM gebruikt kon worden om met de data uit de database te werken.
...
Als er relaties bestaan tussen tabellen en er moet een SQL query geschreven worden dan zou iedere keer volledig de relatie opnieuw geschreven moeten worden in SQL , bij het gebruik van Eloquent word dit gedefinieërd in het model en is daarom veel korter om te schrijven..

Om de naam van het agency van user met id=1 uit de databank te halen wordt volgende ORM methode gebruikt:

`User::find(2)->agency()->name;`

Als dit met een SQL query zou uitgevoerd worden dan zou het er zo uitzien:
...
Zoals weergegeven in bovenstaande code is de syntax in deze eenvoudige query veel overzichtelijker met de ORM. Als er heel veel relaties aanwezig zijn in de applicatie dan zal met de ORM veel minder code geschreven moeten worden en zal er een overzichtelijker project ontstaan dan met SQL queries.
...
om deze relatie te definieren wordt in het Agency model een methode employee gaplaatst: 
...
Vanaf laravel 5.2 kan via artisan gebruik gemaakt worden van de Auth scaffold. Door het command `php artisan make:auth` uit te voeren, zullen alle views gegenereerd worden die nodig zijn voor authenticatie. Deze command zal volgende views genereren: 
...
```
zoals hierboven weergeven wordt zijn er een aantal views gegenereerd die gebruikt kunnen worden in ons project.

Als nu de Route::auth() methode gezet wordt in de routes.php file dan zullen de nodige routes in onze applicatie bestaan om het authenticatie systeem te laten werken. Deze methode staat voor meerdere routes:
```
...
Om het authenticatie systeem aan te passen aan onze applicatie moest de registratie methode in de Authcontroller overschreven worden en de $redirectTo variabele in de Authcontroller aangepast. Deze variabele geeft aan naar waar geredirect word na het inloggen , de uri die in deze variabele wordt gestoken wijst naar een functie in de agency controller die users zal redirecten naar de laatst ingevulde stap. Ook worden de login en reset password views aangepast zodat dit past in het design van het project.
...
HTTP Middleware wordt als filter gebruikt om op bepaald requests een gepaste response te geven. Bv. middleware kan controleren of de persoon die de request stuurt geauthenticeerd is of niet.

Welke middleware zijn er gebruikt in deze applicatie ?


- **web group middleware**<br>
...
Onze applicatie logica word uitgevoerd door de controllers. Zoals uitgelegd bij Routes zullen HTTP requests een specifieke controller functie kunnen uitvoeren. Er zijn 2 "hoofd" controllers :
...
- **show($id)**
als er op een bureau word geklikt in het dashboard dan wordt deze functie  aangesproken in de controller en dan zal de controller een nieuwe view terug sturen namelijk het overzicht van het geklikte agency met alle data die nodig is om dit weer te geven. Het $id verteld de controller op welke agency er is geklikt.
...
Er zijn ook nog aparte controllers voor de resource routes (emloyees , awards, references, clients, comments, strongpoints). In employees en clients zijn er functies voor het aanmaken, updaten, verwijderen, en editeren. Bij awards , references , comments , strongpoints kan er enkel aangemaakt en verwijderd worden.



### Validation

Zoals hierboven vermeld zal de controller de binnenkomende data valideren aan de hand van een aantal regels. Hoe werkt dit nu juist ? Wel, de eerste stap is het definieëren van de validatie regels. Laravel heeft enkele out of the box validatie regels die men kan gebruiken. Hieronder een kort lijstje van de validatie regels die gebruikt zijn in het project.
...
De errors in de view kunnen dan benaderd worden door de $errors variabele te gebruiken.

Er waren problemen hiermee toen 2 dezelfde forms op 1 pagina stonden. De error messages werden op beide forms weergegeven terwijl er maar 1 form ingevuld was. Dit gebeurde in stap 9 waar er vorige en huidige clienten kunnen toegevoegd worden. Het enige verschil tussen deze forms is dat de huidige cliënten geen eindjaar konden invullen. Hiervan is gebruik gemaakt. In de backend word gecheckt of in de request een field zit met eindjaar. Als dit aanwezig is dan worden de validator regels voor vorige clienten in de validator geladen , als dit niet zo is dan de regels voor huidige clienten. Deze Errorbag wordt terug gestuurd naar de frontend met als naam previous voor vorige clienten en als naam current voor huidige clienten. Op deze manier komen de error messages op de juiste plaats en zal er correct gevalideerd worden.



In de view word aan de hand van de Blade templating engine een if geschreven dat als er een error message van een bepaalde field in de errorbag zit, dit field een css klasse met rode border moet krijgen. Hierdoor is makkelijk te zien waar de validatie fout zich bevind in de form.
...
Er is in de views gebruik gemaakt van de built-in templating engine genaamd blade. Als men blade in een view wilt gebruiken dient de view file de blade.php extentie te hebben. Twee grote voordelen hiervan zijn het gebruik van template overerving en het gebruik van secties.

Voor deze applicatie zijn een aantal standaard layouts gemaakt waarvan andere pagina's zullen overerven. 
...
In deze layout files hebben we sections gecreeërd aan de hand van @yield directives. Deze kunnen in de child pages (pages die overerven) gevuld worden aan de hand van @section directives. Deze aanpak zorgt ervoor dat al de code die in de layout files staat maar 1 keer geschreven moet worden terwijl deze voor meerdere pagina's wordt gebruikt. Hieronder een voorbeeld van oververving in deze applicatie.

De** master.blade.php layout** is de layout waar elke pagina van overerft. Het bevat de css/js links, metadata en een yield directive die gebruikt wordt om de inhoud van sections uit child pages te injecteren.
...
Wanneer een view gestuurd wordt naar de client zal de templating engine één view renderen aan de hand van deze 3 blade files.
...
Telkens er een pagina word geladen door de controller zal de controller data meesturen die weergegeven moet worden in de view. Deze data wordt meegestuurd in de vorm van een multidimensionele array genaamd de viewbag.
...
Hoe dit nu weergeven in de view ? Als alle werknemers moeten weergeven worden, wordt met een foreach loop door de array met index employees gelopen.
...
Dit wil zeggen dat voor elke werknemer in de viewbag men de employee view moet includen en in deze employee view wordt de werknemer data gerenderd. De naam wordt weergegeven op deze manier:
...
vermits blade alles dat tussen dubbele curly braces vertaald in echo's hoeft dit niet meer. Dit geeft de template een nettere syntax en zorgt ervoor dat er minder code moet worden geschreven. 
...
Er is ook een rich text editing functie toegevoegd waarbij de content die weergegeven werd niet mocht geëscaped worden omdat de opmaak zichtbaar moest zijn. Hiervoor kan de templating engine niet worden gebruikt omdat deze alle tags escaped, dus om dit op te lossen werd gebruik gemaakt van een package genaamd purifier. Deze package zal alle html tags die gebruikt worden voor de opmaak toelaten en alle andere tags zullen geëscaped worden. Omdat dit in de backend niet goed werkte (cleanen voordat opgeslagen word) wordt de content nog eens gecleaned in de view. Dit wordt op volgende manier uitgevoerd: 

`{!! clean($agency->philosophy) !!}`

##### Lang files

De statische content van de pagina renderen we door het gebruik van language files. Op deze manier is het eenvoudig om meerdere talen toe te voegen aan de applicatie , momenteel was dit nog niet noodzakelijk maar als het toch ooit eens geimplementeerd moet worden is dit niet zoveel extra werk. Een language file bestaat uit een return met een array in met de statische content. 
...
Omdat alle gegevens die worden ingegeven geëditeerd moeten kunnen worden, is ervoor gezorgd dat het form automatisch ingevuld wordt op basis van het object dat meegestuurd word naar de view. Dit is verwezenlijkt aan de hand van model forms. Een model form word geopend op deze manier: 
...
Wanneer een form word ingevuld moet er op de save knop gedrukt worden om de data op te slaan en naar de volgende stap te gaan. Vanaf op deze knop wordt gedrukt zal er een POST HTTP request worden verstuurd naar de server. Aan de hand van de gedeclareerde route URI's zal een functie in de controller worden uitgevoerd. Om met de request te werken moet een instantie van de Request klasse geïnjecteerd worden in de controller. Dit wordt gedaan door het te type-hinten in de constructor. 

`public function store($id, $stepId, Request $request)`

Deze instantie biedt ons enkele methodes die gebruikt worden om de input data te behandelen zoals:
...
Om de web applicatie interactief en dynamisch te maken is gebruik gemaakt van javascript/Jquery. Hierdoor zijn onder andere volgende handelingen mogelijk: inputs limiteren, animeren van elementen, reageren op mouseclicks zonder page refresh, berekeningen doen,... Zo waren er heel wat javascript bestanden die samen gebundeld konden worden met behulp van browserify. Dit levert beter onderhoudbare code op door het opsplitsen van verschillende javascript files die uiteindelijk gecompileerd worden naar 1 main javascript bestand. Dit laat de ontwikkelaar toe om op een modulaire manier code te schrijven. Hoe gebeurt dit nu juist ? Door een main.js bestand in de root van de javascript folder te plaatsen. Hierin requiren we andere javascript files. Als nu gulp wordt gerunt dan zal browserify al deze javascript files compileren in 1 groot javascript bestand.

Welke verschillende javascript files zijn er allemaal gebundeld en wat is hun functie in het project ? 
...
- **clients.js**
animatie voor het weergeven van het clients form om een client toe te voegen + het toevoegen van een has-errors klasse om een red border weer te geven bij validatie fout , hier moest dit gedaan worden met javascript omdat dit niet werkte bij de multiselect op de normale manier (via de templating engine).
- **dashboard.js**
...
- **imagePreview.js**
Zorgt ervoor dat als de user een image selecteerd dat deze instant gepreviewd wordt op de pagina.
- **jquery-ui.min.js**
...
Wanneer nu een geregistreerd communicatie bureau zijn wachtwoord wilt aanpassen dan zal er een extra field staan namelijk Old password zodat eerst het vorige passwoord moet worden ingegeven voordat het pas verandert kan worden.
